  ...
  return <div className="max-w-7xl mx-auto px-4 py-5 md:py-6">
    ...
    {loading && !data ? <div className="py-24 text-center"><Loader2 size={28} className="animate-spin text-accent mx-auto" /><div className="text-xs text-text-secondary mt-3">正在配对交易日志与历史行情</div></div> : data && <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
        <section className="border border-border rounded-md p-4"><div className="flex items-center justify-between"><h2 className="text-sm font-semibold text-text">股票贡献</h2><span className="text-[11px] text-text-secondary">权重 × 区间收益</span></div>{data.by_stock.length ? <div className="mt-3 space-y-3">{data.by_stock.slice(0, 12).map((item) => { const width = Math.min(Math.abs(item.contribution_pct || 0) * 12, 100); return <div key={item.code}><div className="flex items-center justify-between gap-3 text-xs"><span className="text-text truncate">{item.name}<span className="font-mono text-text-secondary ml-2">{item.code}</span></span><span className={`font-mono ${tone(item.contribution_pct)}`}>{signed(item.contribution_pct)}</span></div><div className="relative h-1.5 bg-[#21262D] mt-1.5 rounded overflow-hidden"><div className={`absolute h-full ${Number(item.contribution_pct) >= 0 ? 'bg-up' : 'bg-down'}`} style={{ width: `${width}%`, left: Number(item.contribution_pct) >= 0 ? 0 : undefined, right: Number(item.contribution_pct) < 0 ? 0 : undefined }} /></div><div className="text-[10px] text-text-secondary mt-1">仓位 {number(item.weight_pct, 1)}% · 区间 {signed(item.return_pct)} · {item.source}</div></div>; })}</div> : <Empty text="持仓或历史行情不足" />}</section>
