      ...
      {data && <>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_300px] gap-5">
          <main className="min-w-0">
            ...
            {activePool === 'etf' && compareItems.length >= 2 && <section className="border border-accent/50 rounded-md p-3 mb-4 overflow-x-auto"><div className="flex items-center justify-between gap-3 mb-3"><div><h3 className="text-sm font-semibold text-text">ETF快速对比</h3><p className="text-[11px] text-text-secondary mt-1">最多选择4只，比较实时价格、波动和技术状态</p></div><button type="button" onClick={() => setCompareCodes([])} className="text-xs text-text-secondary hover:text-text">清空选择</button></div><table className="w-full min-w-[560px] text-xs"><thead className="text-text-secondary border-b border-border"><tr><th className="text-left py-2">ETF</th><th className="text-right py-2">现价</th><th className="text-right py-2">涨跌幅</th><th className="text-right py-2">换手率</th><th className="text-right py-2">RSI6</th><th className="text-right py-2">量比</th></tr></thead><tbody>{compareItems.map((item) => <tr key={item.code} className="border-b border-border/50"><td className="py-2 text-text"><StockKlineButton code={item.code} name={item.display_name} className="text-text">{item.display_name}<span className="text-text-secondary font-mono ml-2">{item.code}</span></StockKlineButton></td><td className="py-2 text-right font-mono">{item.price == null ? '--' : numberText(item.price)}</td><td className={`py-2 text-right font-mono ${getChangeColor(item.change_pct || 0)}`}>{signed(item.change_pct)}</td><td className="py-2 text-right font-mono text-text-secondary">{numberText(item.turnover)}</td><td className="py-2 text-right font-mono text-text-secondary">{numberText(item.technical?.rsi?.rsi6, 1)}</td><td className="py-2 text-right font-mono text-text-secondary">{numberText(item.technical?.volume?.ratio)}</td></tr>)}</tbody></table></section>}
            {active.length === 0 ? <div className="border border-border rounded-md py-16 text-center text-sm text-text-secondary"><Bookmark size={24} className="mx-auto mb-2 text-border" />{query || etfType ? '没有匹配的标的' : '这个池暂时为空'}<div className="text-xs mt-2">可以从智能选股、量化信号或技术筛选直接加入。</div></div> : <div className="grid grid-cols-1 md:grid-cols-2 gap-3">{active.map((item) => <article key={item.id} className="border border-border rounded-md bg-card p-4 hover:border-accent/60 transition-colors">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0"><div className="flex items-center gap-2"><StockKlineButton code={item.code} name={item.display_name} className="min-w-0 truncate font-semibold text-text">{item.display_name}</StockKlineButton><span className={`text-[11px] border rounded px-1.5 py-0.5 ${item.status === 'holding' ? 'border-down/50 text-down' : item.status === 'reduce' ? 'border-up/50 text-up' : 'border-border text-text-secondary'}`}>{STATUS_LABELS[item.status] || item.status}</span></div><div className="text-xs text-text-secondary mt-1 font-mono">{item.code}<span className="font-sans ml-2">{item.industry || item.sector || '行业未标注'}</span>{item.asset_type === 'etf' && item.etf_type ? <span className="ml-2 text-accent">{item.etf_type}</span> : null}</div></div>
