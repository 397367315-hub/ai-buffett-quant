    ...
    <div className="max-w-7xl mx-auto px-4 py-6 overflow-x-hidden">
      ...
      {!loading && data && <>
        ...
        {data.stocks.length ? <section className="border border-border bg-card rounded-lg overflow-hidden">
          <div className="overflow-x-auto"><table className="w-full min-w-[1120px] text-sm"><thead><tr className="text-text-secondary border-b border-border bg-[#0D1117] text-xs"><th className="text-left px-4 py-3 font-medium">股票</th><th className="text-right px-3">现价</th><th className="text-right px-3">涨跌幅</th><th className="text-right px-3">量比</th><th className="text-right px-3">换手</th><th className="text-right px-3">PE / PB</th><th className="text-right px-3">ROE</th><th className="text-right px-3">主力净流入</th><th className="text-right px-3">成交额</th><th className="text-right px-3">市值</th><th className="text-right px-3">个人池</th></tr></thead><tbody>{data.stocks.map((stock) => <tr key={stock.code} className="border-b border-border/50 hover:bg-[#21262D]"><td className="px-4 py-3"><StockKlineButton code={stock.code} name={stock.name} className="text-text font-medium">{stock.name}</StockKlineButton><div className="text-xs text-text-secondary mt-0.5">{stock.code}{stock.sector ? ` · ${stock.sector}` : ''}</div></td><td className="px-3 py-3 text-right font-mono text-text">{number(stock.price)}</td><td className={`px-3 py-3 text-right font-mono ${getChangeColor(Number(stock.change_pct))}`}>{Number(stock.change_pct) >= 0 ? '+' : ''}{number(stock.change_pct)}%</td><td className="px-3 py-3 text-right font-mono text-text">{number(stock.volume_ratio)}</td><td className="px-3 py-3 text-right font-mono text-text-secondary">{number(stock.turnover)}%</td><td className="px-3 py-3 text-right font-mono text-text-secondary">{number(stock.pe, 1)} / {number(stock.pb, 1)}</td><td className="px-3 py-3 text-right font-mono text-text-secondary">{number(stock.roe, 1)}%</td><td className={`px-3 py-3 text-right font-mono ${getChangeColor(Number(stock.main_net_inflow))}`}>{formatYi(Number(stock.main_net_inflow || 0))}</td><td className="px-3 py-3 text-right font-mono text-text-secondary">{number(Number(stock.amount || 0) / 1e8)}亿</td><td className="px-3 py-3 text-right font-mono text-text-secondary">{number(Number(stock.market_cap || 0) / 1e8, 0)}亿</td><td className="px-3 py-3 text-right"><AddToPersonalPoolButton code={stock.code} name={stock.name} industry={stock.sector} thesis={`技术筛选：${data.criteria.preset}，涨跌幅${number(stock.change_pct)}%，量比${number(stock.volume_ratio)}，换手率${number(stock.turnover)}%，主力净流入${formatYi(Number(stock.main_net_inflow || 0))}`} source="technical_screener" compact /></td></tr>)}</tbody></table></div>
