...
interface MacroData {
  ...
  cache_used: boolean;
  global_markets: Array<{ key: string; label: string; value: number | null; change_pct: number | null; change_kind?: string; currency: string; source_time: string | null; available: boolean; source: string; is_realtime?: boolean; cache_used?: boolean; data_age_minutes?: number | null }>;
  macro_indicators: Record<string, { key: string; label: string; value: number | null; pulse_pct?: number | null; yoy_pct?: number | null; mom_pct?: number | null; latest_monthly_increment?: number | null; source_time: string | null; available: boolean; source: string; cache_used?: boolean; note?: string }>;
...
const pointChange = (value: number | null | undefined) => value == null ? '--' : `${value >= 0 ? '+' : ''}${value.toFixed(3)}个百分点`;
...
const marketChange = (item: { change_pct: number | null; change_kind?: string }) => item.change_kind === 'change_points' ? pointChange(item.change_pct) : signed(item.change_pct);
...
export default function MacroPage() {
  ...
  return <div className="max-w-7xl mx-auto px-4 py-5 md:py-6">
    ...
    {loading && !data ? <div className="py-24 text-center"><Loader2 size={28} className="animate-spin text-accent mx-auto" /><div className="text-sm text-text mt-4">正在汇总国际市场与国内政策</div><div className="max-w-sm mx-auto h-1.5 bg-[#21262D] mt-5 rounded overflow-hidden"><div className="h-full bg-accent transition-all" style={{ width: `${progress}%` }} /></div><div className="text-xs font-mono text-text-secondary mt-2">{progress}%</div></div> : data && <>
      ...
      <section className="mb-5"><h2 className="text-sm font-semibold text-text flex items-center gap-2 mb-3"><Globe2 size={16} className="text-accent" />全球市场</h2><div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 border border-border rounded-md divide-x divide-y lg:divide-y-0 divide-border">{data.global_markets.map((item) => <div key={item.key} className="p-3 min-w-0"><div className="flex items-center justify-between gap-2"><div className="text-[11px] text-text-secondary truncate">{item.label}</div><span className={`shrink-0 text-[9px] ${item.is_realtime ? 'text-down' : item.cache_used ? 'text-warn' : 'text-text-secondary'}`}>{item.is_realtime ? '实时源' : item.cache_used ? '缓存' : item.key.startsWith('us') || item.key === 'vix' ? '最新发布' : '最新报价'}</span></div><div className="font-mono text-base text-text mt-1 truncate">{number(item.value, item.value && item.value < 1000 ? 2 : 1)}</div><div className={`font-mono text-xs mt-1 ${tone(item.change_pct)}`}>{marketChange(item)}</div><div className="text-[10px] text-text-secondary mt-1 truncate" title={item.source_time || undefined}>{item.source_time || '源时间未返回'}</div><div className="text-[9px] text-text-secondary mt-1 truncate">{item.source}{item.data_age_minutes != null ? ` · ${item.data_age_minutes.toFixed(0)}分钟前` : ''}</div></div>)}</div></section>

      <section className="border border-border rounded-md p-4 mb-5"><div className="flex flex-wrap items-start justify-between gap-2"><div><h2 className="text-sm font-semibold text-text flex items-center gap-2"><Landmark size={15} className="text-accent" />补充宏观指标</h2><p className="text-[10px] text-text-secondary mt-1">最新发布值参与 V5 因子计算；指标发布频率不同，不代表盘中实时。</p></div><span className="text-[10px] text-text-secondary">缺失源只回退对应历史快照</span></div><div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-4"><MacroIndicatorCard item={data.macro_indicators?.credit_pulse} title="信用脉冲代理" kind="credit" /><MacroIndicatorCard item={data.macro_indicators?.industry_price} title="产业价格信号" kind="price" /><MacroIndicatorCard item={data.macro_indicators?.capex} title="资本开支宏观代理" kind="capex" /></div></section>
...
function MacroIndicatorCard({ item, title, kind }: { item?: MacroData['macro_indicators'][string]; title: string; kind: 'credit' | 'price' | 'capex' }) {
  if (!item?.available) return <article className="border border-border rounded-md p-3"><div className="text-xs text-text">{title}</div><div className="text-sm text-text-secondary mt-3">当前无可核验发布值</div><div className="text-[10px] text-text-secondary mt-1">系统不会用 0 或中性值替代</div></article>;
  const value = kind === 'credit' ? item.pulse_pct ?? item.value : item.yoy_pct;
  const subtitle = kind === 'credit' ? `最新月度社融增量 ${item.latest_monthly_increment ?? '--'}` : `环比 ${signed(item.mom_pct)}`;
  return <article className="border border-border rounded-md p-3 min-w-0"><div className="flex items-start justify-between gap-2"><div className="text-xs text-text">{title}</div><span className={`text-[9px] ${item.cache_used ? 'text-warn' : 'text-text-secondary'}`}>{item.cache_used ? '缓存' : '最新发布'}</span></div><div className={`font-mono text-lg mt-3 ${tone(value)}`}>{value == null ? '--' : signed(value)}</div><div className="text-[10px] text-text-secondary mt-1">同比/代理变化 · {subtitle}</div><div className="text-[10px] text-text-secondary mt-2 truncate" title={item.source_time || undefined}>{item.source_time || '--'} · {item.source}</div>{item.note && <p className="text-[10px] leading-4 text-warn mt-2">{item.note}</p>}</article>;
}
