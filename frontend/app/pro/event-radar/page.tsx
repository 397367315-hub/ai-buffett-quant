...
function eventStatusLabel(status: string | undefined): string {
  if (status === 'CONFIRMED') return '事件雷达确认';
  if (status === 'POSSIBLE') return '事件雷达疑似';
  if (status === 'FORMING') return '事件雷达形成中';
  return status || '待确认';
}
