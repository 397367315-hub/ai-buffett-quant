...
export default function SignalList({ snapshot, job, onRefresh, onAddToPaper, history }: {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border pb-3">
        <div>
          ...
          <div className="text-xs text-text-secondary mt-1 flex flex-wrap items-center gap-x-3 gap-y-1">
            ...
            {snapshot?.is_realtime && <span className="inline-flex items-center gap-1 text-up"><Wifi size={12} />盘中实时</span>}
          </div>
        </div>
      ...
      {job?.status === 'failed' && <div className="border border-down/50 bg-[#EF535022] rounded-md p-3 text-xs text-down flex items-start gap-2"><AlertTriangle size={15} className="shrink-0" />{job.error || '扫描失败，请稍后重试。'}</div>}
      ...
      {snapshot && <div className="grid grid-cols-2 sm:grid-cols-4 border border-border rounded-md divide-x divide-border">
        ...
        <div className="p-3"><div className="text-xs text-text-secondary">数据状态</div><div className={`text-sm mt-1 ${!snapshot.generated_at ? 'text-text-secondary' : snapshot.stale ? 'text-warn' : 'text-up'}`}>{!snapshot.generated_at ? '未扫描' : snapshot.stale ? '缓存降级' : '可用'}</div></div>
      </div>}
      ...
      <section className="border border-border rounded-md overflow-hidden">
        ...
        {signals.length ? <div className="overflow-x-auto"><table className="w-full min-w-[860px] text-xs">
          ...
          <tbody>{signals.map((signal) => <tr key={signal.signal_id} className="border-t border-border/70 hover:bg-[#161B22]">
            ...
            <td className="px-3 py-2.5 text-text-secondary max-w-[240px]"><div className="line-clamp-2">{signal.matched_rules.join('；')}</div></td>
