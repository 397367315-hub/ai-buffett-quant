...
export default function ZhabanPanel() {
  const [factors, setFactors] = useState<ZhabanFactor[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    const responses = await Promise.allSettled([
      apiFetch<{ data: { strategy: Config; factors: ZhabanFactor[] } }>('/quant/zhaban/config'),
      apiFetch<{ data: ZhabanResearch | null }>('/quant/zhaban/latest'),
      apiFetch<{ data: ZhabanBacktest | null }>('/quant/zhaban/backtest/latest'),
    ]);
    const [configResponse, researchResponse, backtestResponse] = responses;
    if (configResponse.status === 'fulfilled') { setConfig(configResponse.value.data.strategy || {}); setFactors(configResponse.value.data.factors || []); }
    if (researchResponse.status === 'fulfilled') setResearch(researchResponse.value.data);
    if (backtestResponse.status === 'fulfilled') setBacktest(backtestResponse.value.data);
    if (responses.every((item) => item.status === 'rejected')) setError('炸板研究接口暂时无法连接');
    setLoading(false);
  }, []);
  ...
  const runScan = async () => {
    ...
    } catch (caught) { setError(caught instanceof Error ? caught.message : '炸板扫描启动失败'); }
  };
  ...
  const runBacktest = async () => {
    ...
    } catch (caught) { setError(caught instanceof Error ? caught.message : '炸板回测启动失败'); }
  };
