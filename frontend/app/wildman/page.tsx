...
function DataDates({ payload, refreshing }: { payload: AnyMap; refreshing: boolean }) {
  const source = payload.source_status || {};
  const dateText = (value: unknown) => {
    const text = String(value || '').trim();
    return /^\d{8}$/.test(text) ? `${text.slice(0, 4)}-${text.slice(4, 6)}-${text.slice(6, 8)}` : safe(text, '未返回');
  };
  const updated = new Date(payload.updated_at || '');
  const updatedText = Number.isNaN(updated.getTime()) ? '--' : new Intl.DateTimeFormat('zh-CN', {
    timeZone: 'Asia/Shanghai', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hourCycle: 'h23',
  }).format(updated);
  return <div className="mb-4 border-b border-border pb-3 text-[11px] leading-5 text-text-secondary" aria-live="polite">
    <div className="flex flex-wrap gap-x-5 gap-y-1">
      <span>分析交易日 <time className="font-mono text-text">{dateText(payload.trade_date)}</time></span>
      <span>行情日期 <time className="font-mono text-text">{dateText(source.data_date)}</time>{source.limit_up && <span> · {sourceText(source.limit_up)}</span>}</span>
      <span>情绪日报 <time className="font-mono">{dateText(source.sentiment_date)}</time></span>
      <span>{refreshing ? '正在获取最新数据' : `刷新时间 ${updatedText}`}</span>
    </div>
    {source.sentiment_current === false && <p className="mt-1 text-warn">情绪日报尚未更新到本次交易日，最新行情独立更新。</p>}
  </div>;
}
...
function sourceText(value: unknown): string {
  ...
  const labels: Record<string, string> = {
    numcat: '猫爪 NumCat',
    numcat_limit_pool: '猫爪·涨跌停池',
    eastmoney_limit_pool: '东方财富·涨跌停池',
